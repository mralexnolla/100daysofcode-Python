...
class Snake:
    ...
    def extend(self):
        ...
        last_segment = self.segmentList[-1]
        last_x = last_segment.xcor()
        last_y = last_segment.ycor()

        # Calculate the new position based on the direction the snake is moving
        direction = self.head.heading()
        if direction == UP:
            new_x = last_x
            new_y = last_y - MOVE_DISTANCE
        elif direction == DOWN:
            new_x = last_x
            new_y = last_y + MOVE_DISTANCE
        elif direction == LEFT:
            new_x = last_x + MOVE_DISTANCE
            new_y = last_y
        else:
            new_x = last_x - MOVE_DISTANCE
            new_y = last_y
